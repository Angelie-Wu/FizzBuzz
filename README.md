...
- For loop in Python use Range to decide the number of iterates and Java use an condition in the middle part that 
follows the and follow by the semicolon
- for loop in Java has a counter variable that can only be accessed in the for loop itself. However, Python don't have 
this.
- The code in the for loop in Java needs to be inside of the open and close curly brackets. In Python, the codes are 
using indent to separate from other code.
- The for loop run the same way in both Python and Java which iterates the code in the loop body for several number of 
iterations.
- The for loop in Java have "i++" which adds up the counter variable "i" by one after all the loop body codes were run 
in each iteration and the variable "i" can be use in the loop body code. In Python, the loop automatically run and count
the numbers of iterations. If we want to use the counter of the iterates in each run, we need to add an extra variable 
to do so.   
